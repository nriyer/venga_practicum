...
rsvp_pos_fact$newyears_flag <- with(rsvp_pos_fact,ifelse((month_visit == 12 & 
...
rm(server_source,server_repeats,server_counts)
```
...
rm(table_source,table_repeats,table_counts)
```

## 9. Creating aggregate view by Loyalty User 
Combines the aggregates from rvsp, pos, server stats, table stats, and first visits features
...
rm(aggr_rsvp_pos,first_rsvp_pos,fact_server,fact_table)
...
Creates two data frames: one of pos items by reservation and one of post items by user
the user pos items table is an average of loyalty spend category
```{r}
...
Creates 3 dataframes: aniversary codes and birthday by reservation, total aniversary and birthday by loyalty user id, and 
sum of the state cancelled and no show by loyatly user id. 

I used cmb_rsvp_pos for code to ensure that the cancelled and no shows are counted. Otherwise, there are more reservations than in pos. 
```{r}
#code by reservation
code <- cmb_rsvp_pos %>%
                group_by(loyalty_user_id,loyalty_reservation_id) %>%
...
Creating three dataframes: difference in days between visit and column identifying the visit number by reservation id and average days between visits by loyalty user id, and whether guest repeated on the same visit. 
...
Creates two dataframes: dataframe that merges on the intermediate loyalty user data frames and another that merges intermediat loyatly reservation data frames. 

```{r}

user_view <- merge(view_aggr_rsvp_pos,items_aggregates,by="loyalty_user_id")
...
write.csv(user_view,row.names = FALSE,"user_view_3865.csv")
